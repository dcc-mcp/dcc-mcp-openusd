"""Create a DistantLight prim (pxr-required)."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a DistantLight prim.")
    parser.add_argument("--stage-file", required=True)
    parser.add_argument("--prim-path", required=True)
    parser.add_argument("--angle", type=float, default=0.53)
    ...
    try:
        ...
        print(json.dumps({"success": True, "message": "Distant light created", **result}))
        return 0
