...
| `variant_nested.usda` | none (`valid: true`) | A `variantSet` nested inside another. Each nested set's variant entries must be claimed only by its own scan — otherwise the brace discount over-subtracts, prims land one level too high and the binding looks dangling (error, flips `valid`). |
